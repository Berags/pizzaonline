...
const componiTabella = (carrello) => {
	carrello.filter((el) => {
		...
		<img src="./static/images/menu/${element.imgpath}" class="w-10 h-10 object-cover rounded-md" alt="">
		<span class="ml-4 font-semibold text-sm">${element.nome}</span>
		</div>
		<div class="w-32 flex justify-between">
		<input type="hidden" value="${element.id}">
		<span class="px-3 py-1 rounded-md bg-gray-300" onclick="rimuoviDaCarrello(${index}, ${element.prezzo})">-</span>
		<input id="input-${index}" name="quantita[]" readonly class="font-semibold mx-4 w-2" value="${element.quantita}">
		<span class="px-3 py-1 rounded-md bg-gray-300" onclick="aggiungiDaCarrello(${index}, ${element.prezzo})">+</span>
		</div>
